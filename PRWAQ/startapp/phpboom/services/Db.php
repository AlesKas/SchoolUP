...
class Db {
    ...
    public function __construct(){
        ...
        if ($sqlconnection->query('SELECT 1 FROM `users`') === FALSE)
        {
            $sql = file(__DIR__.'/test.sql');
            Db::createTable($sql, $sqlconnection);
        }
        if ($sqlconnection->query('SELECT 1 FROM `cars`') === FALSE)
        {
            $sql = file(__DIR__.'/cars.sql');
            Db::createTable($sql, $sqlconnection);
        }
    }

    private function createTable($sql, $sqlconnection)
    {
        $query = '';
        foreach ($sql as $line)	{
            ...
            $query = $query . $line;
            if ($endWith == ';') {
                mysqli_query($sqlconnection,$query) or die('<div class="error-response sql-import-response">Problem in executing the SQL query <b>' . $query. '</b></div>');
                $query= '';		
            }
        }
    }
